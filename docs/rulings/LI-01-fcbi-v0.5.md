...
| **W3-05** | MAJOR | Every convergence depth reruns `float_paths` from scratch; the ceiling probe (513 feeders) did not complete quickly. | **FIXED (v0.5.4), then CORRECTED (v0.5.5) — see the wave-4 disposition below.**  v0.5.4 replaced the restart with a lazy best-first generator and a frontier bound; wave-4 proved that generator was **not** `float_paths`-equivalent and its "120 random networks" check insufficient (a 500-DAG corpus later found the divergence).  The best-first variant was **withdrawn**: `iter_float_paths` now streams the EXACT `float_paths` sequence and the frontier is read off `float_paths`'s own used set (W4-01/02).  The performance goal of W3-05 still holds via the sound frontier + cap — far-off-critical fans stop in ms; a pathological wide near-critical fan is slower but capped/provisional (correctness-first).  Do NOT read the v0.5.4 timings / "identical on 120 networks" line above as current. |
...
## Port note ? canonical v0.4.6 lineage

Ported onto `worktodo77/schedule-iq` v0.4.6 lineage for the v0.5.0 release.  The
canonical implementation reconciles target resolution and per-window activity
matching to the R-ID UID-first identity rule (UID-stable re-codes are not
replacements; code is display-only with the narrow legacy fallback).  This
supersedes the matrix branch's code-first RW3-F2 behavior.  A mixed-calendar
frontier fixture is included to verify that the native-hour frontier stop agrees
with exhaustive fixed-reference-hour distances.  The required scorecard re-pin
was run after the port: because LI-01 is provisional/ungraded pending anchor
recalibration, the pinned demo grades hold at C+/C/D per file and D for the
series.
