...
def _li02_score(sa: SeriesAnalysis, override: dict) -> tuple:
    """Design-specified LHL mapping (branches on churn, per audit rulings
    L1 + L10, v0.4.5).  Offender count = relationship deaths (changed ties).

    - Months basis unavailable (missing / non-increasing data dates):
      UNGRADEABLE — never scored by any branch (pre-0.4.5, a reached median
    ...
    - Median not reached: 100 when fewer than ``deaths_pass_threshold`` of
      ...
      half-life); else the 70-point placeholder.  (Pre-0.4.5 this branch
      tested the CENSORED fraction — inverted vs its own published
      rationale; a frozen network scored 70.)  The value reported alongside
      a not-reached score is the longest-follow-up lower bound.
    """
    try:
